...
API_KEY_LLAMA = os.environ.get("GROQ_API_KEY") or os.environ.get("API_KEY_LLAMA")
...
def _local_path_from_uploaded_url(file_source: str) -> str:
    """
    Convert the local upload URL (e.g., http://127.0.0.1:8000/uploads/foo.pdf)
    back into a filesystem path under the uploads folder.
    """
    if file_source.startswith("http://127.0.0.1:8000/uploads/"):
        return os.path.join(UPLOAD_FOLDER, os.path.basename(file_source))
    return file_source


def input_pdf(file_source):
    try:
        # Normalize to local path when we get back our own upload URL
        file_source = _local_path_from_uploaded_url(file_source)

        # DOCX handling
        if file_source.lower().endswith(".docx"):
            if not os.path.exists(file_source):
                raise FileNotFoundError("DOCX file not found.")
            doc = Document(file_source)
            text = "\n".join(p.text for p in doc.paragraphs)
            if not text.strip():
                raise ValueError("No text found in the DOCX.")
        ...
        if file_source.lower().endswith(".txt"):
            ...
            with open(file_source, "r", encoding="utf-8", errors="ignore") as f:
                text = f.read()
            if not text.strip():
                raise ValueError("No text found in the TXT.")
            return [{"mime_type": "text/plain", "data": text}]
...
46. Complete course in AutoCAD - https://www.udemy.com/course/complete-course-in-autocad-2d-and-3d/
47. AutoCAD 2024: Complete Beginners to Advanced - https://www.udemy.com/course/autodesk-autocad-masterclass-the-ultimate-guide-to-autocad/
48. GD&T Basics - https://www.udemy.com/course/gdt-basics/
49. Manufacturing Guide for Mechanical Product Development -https://www.udemy.com/course/manufacturing-guide-for-mechanical-product-development/
...
def generate_llama_response2(result, predefined):
    full_prompt = f"""
You are generating a study-plan recommendation text to be parsed by a simple frontend.
STRICT FORMAT (no markdown, no bullets other than the numbered list):
- Start with 1 short paragraph (1-3 sentences) summarizing the key focus areas.
- Then give a numbered list of 5-8 courses, each on its own line, format EXACTLY:
  1. Course Name - https://link
- No bold, no extra punctuation, no headers, no trailing text.

Use the analysis_result and the predefined catalog below to choose the most relevant courses.
...
def generate_llama_response(prompt, pdf_content, query):
    if not API_KEY_LLAMA:
        raise ValueError("Missing GROQ_API_KEY (or API_KEY_LLAMA) environment variable.")
...
def chat1():
    ...
    try:
        ...
        logging.error("Error in /chat1: %s", traceback.format_exc())
...
def job():
    ...
    try:
        ...
        logging.error("Error in /job-description: %s", traceback.format_exc())
        return jsonify({"error": str(e)}), 500
...
def chat():
    ...
    try:
        ...
        logging.error("Error in /chat: %s", traceback.format_exc())
        return jsonify({"error": str(e)}), 500
...
def study():
    ...
    try:
        ...
        logging.error("Error in /study-plan: %s", traceback.format_exc())
        return jsonify({"error": str(e)}), 500
